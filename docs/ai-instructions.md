...
**QPointer safety:**
```cpp
// ALWAYS check QPointer before dereferencing
...
void someFunction() {
    ...
    mpHost->doSomething();  // Safe to use
}
```
...
class MyClass {
    ...
    ~MyClass() {
        // ALWAYS stop timers to prevent callbacks on deleted objects
        if (mTimer) {
            mTimer->stop();
        }
    }
...
**Safe deferred execution with QTimer::singleShot:**
```cpp
// BAD - lambda may execute after 'this' is deleted
QTimer::singleShot(0, [this]() {
    mpHost->doSomething();  // CRASH if this/mpHost deleted
});

// GOOD - pass receiver object for automatic cancellation
QPointer<Host> hostGuard = mpHost;
QPointer<TConsole> consoleGuard = mpConsole;
QTimer::singleShot(0, mpConsole, [this, hostGuard, consoleGuard]() {
    if (!hostGuard || !consoleGuard) {
        return;  // Objects were deleted, abort safely
    }
    hostGuard->doSomething();
});
```

**Key principles:**
- Never assume QPointer members are valid - always check before use
- Stop all timers in destructors to prevent use-after-delete
- Use receiver object (2nd parameter) with `QTimer::singleShot` for automatic cleanup
- Capture QPointer guards in lambdas, never assume `this` or members remain valid

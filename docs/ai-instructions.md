...
Scripting is Lua 5.1, not 5.4 - no `goto`, no integer division or bitwise operators, and `unpack` rather than `table.unpack`. `src/lua-function-list.json` is autogenerated, do not update it.

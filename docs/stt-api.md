...
| `stt.cancel()` | `true` \| `nil, error` | Stop listening and **discard**: the utterance in progress is thrown away, no `sysSTTResult` follows for it, and the state returns to `ready`. For leaving rather than finishing - a package that stops because its profile is no longer in front has no use for the half-sentence `stt.stop()` would send. A phrase still being finalised (`processing`, on a backend that finalises after its stop returns) is discarded too, and a request still waiting on a permission dialog is withdrawn - the system dialog stays up, but its answer no longer opens the microphone. Owned the way `stt.stop()` is: refused from a profile that does not hold the microphone. Cancelling when nothing is listening succeeds; cancelling in `error` returns `nil` and a message. A cancel that ends something is announced only by the state change - the call was this profile's own, so its caller already knows, and a package that shows a microphone control puts it back on `ready` either way. |
...
a session that is running perfectly well to a game that cannot act on it. And

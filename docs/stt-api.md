...
- **One recognizer per client.** There is one microphone and one decoder,
  shared across profiles, so at most one profile is listening at a time. Events
  are raised on the profile **holding the microphone**, or on the active profile
  when no session is running; routing text anywhere else is consumer policy, not
  bridge behaviour.
...
| `stt.start()` | `true` \| `nil, error` | Begin listening. From a profile that is not the one holding the microphone this asks for it, which ends the other profile's session - so it can return `nil, error` where a single-profile client would always have succeeded, and is refused outright while the outgoing profile's last phrase is still being decoded. `true` means the request was accepted, not always that audio is already flowing: a client that must ask permission first reports `starting`, and the outcome arrives as `sysSTTStateChanged`. A request refused outright — no model, a phrase still processing, a microphone that will not open, permission already denied — returns `nil` and an error, with the detail in `sysSTTError`. Starting while already listening, or while `starting`, succeeds without asking twice. |
| `stt.stop()` | `true` \| `nil, error` | Only the profile holding the microphone may stop it; from any other it is refused, since ending a session another game started would leave that game with nothing but a state change to explain it. Stop listening and **finalise**: remaining audio is decoded and reported via `sysSTTResult`, and the state reaches `ready`. Whether it has got there by the time the call returns depends on the engine, the way `start()`'s `starting` does: Vosk and sherpa-onnx decode the remainder inside the call, so the state is already `ready`; the built-in macOS backend returns while still `processing` and settles when its recognition task reports back (or after its finalise timeout), with `sysSTTStateChanged` announcing it. A package must not assume the first shape — `stt.stop()` immediately followed by `stt.start()`, and two `stt.toggle()` calls in quick succession, are **refused on macOS**, because starting from `processing` is refused. Wait for `sysSTTStateChanged` to report `ready`. Stopping when nothing is listening succeeds; stopping in `error` returns `nil` and a message, since "stopped" and "was never running because it failed" are different answers. |
| `stt.cancel()` | `true` \| `nil, error` | Stop listening and **discard**: nothing heard so far is delivered, no `sysSTTResult` follows, and the state returns to `ready`. For leaving rather than finishing - a package that stops because its profile is no longer in front has no use for the half-sentence `stt.stop()` would send. A phrase still being finalised (`processing`, on a backend that finalises after its stop returns) is discarded too, and a request still waiting on a permission dialog is withdrawn. Owned the way `stt.stop()` is: refused from a profile that does not hold the microphone. Cancelling when nothing is listening succeeds; cancelling in `error` returns `nil` and a message. Only the state change announces it - the call was this profile's own, so its caller already knows, and a package that shows a microphone control puts it back on `ready` either way. |
...
| `listening` | boolean | **This profile** is capturing now, as `stt.listening()` — not the shared engine's state. `state` below is the engine's, so the two differ while another profile is listening: `listening` is `false` here and `state` reads `"listening"`. |
...
`stt.stop()`, `stt.cancel()` and `stt.close()` can all be refused while another profile is
...
A client with windows of its own also marks the window holding the listening
profile - any profile it holds, not only the one it is showing - so an open
microphone stays visible when that window is minimised or behind something
else — the case a "keep listening while another application is
...
1. **Stop finalises; only a cancel, errors and engine artifacts discard.**
   ...
   `sysSTTResult`. No path silently drops recognised speech except
   `stt.cancel()`, which a script on the owning profile asked for, a fault,
   which reports via `sysSTTError` — and what the engine produced from

...
private slots:
    ...
    void cleanupTestCase()
    {
        ...
        if (mpHost) {
            const QString replayDir = MudletApp::getMudletPath(enums::profileReplayAndLogFilesPath, mHostname);
    ...
    void cleanup()
    {
        ...
        if (mpHost->mTelnet.recordingReplay()) {
    ...
    void test_failedLogStartLeavesTheButtonUnchecked()
    {
        ...
        QVERIFY2(!QFileInfo::exists(MudletApp::getMudletPath(enums::profileDataItemPath, mpHost->getName(), qsl("autolog"))), "a failed log start left the autolog sentinel behind");
    ...
    {
        const QString sentinel = MudletApp::getMudletPath(enums::profileDataItemPath, mpHost->getName(), qsl("autolog"));

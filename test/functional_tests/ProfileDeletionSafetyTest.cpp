...
private:
    ...
    QString profilePath(const QString& profile) const { return utils::getMudletPath(enums::profileHomePath, profile); }

    void makeProfileWithSavedGame(const QString& profile) const
    {
        QVERIFY(QDir().mkpath(utils::getMudletPath(enums::profileXmlFilesPath, profile)));
        QFile savedGame(qsl("%1/%2.xml").arg(utils::getMudletPath(enums::profileXmlFilesPath, profile), profile));
...
private slots:
    ...
    void test_removingCurrentDirectoryProfileKeepsEveryProfile()
    {
        ...
        QVERIFY2(QDir(utils::getMudletPath(enums::profilesPath)).exists(), "the profiles directory was deleted");
        QVERIFY2(QDir(profilePath(mKeeper)).exists(), "an unrelated profile was deleted");
        QVERIFY2(QFile::exists(qsl("%1/%2.xml").arg(utils::getMudletPath(enums::profileXmlFilesPath, mKeeper), mKeeper)), "an unrelated profile's saved game was deleted");
    ...
    void test_removingParentDirectoryProfileKeepsTheConfigurationDirectory()
    {
        ...
        QVERIFY2(QDir(utils::getMudletPath(enums::mainPath)).exists(), "Mudlet's configuration directory was deleted");
        QVERIFY2(QDir(utils::getMudletPath(enums::profilesPath)).exists(), "the profiles directory was deleted");
        QVERIFY2(QDir(profilePath(mKeeper)).exists(), "an unrelated profile was deleted");
        QVERIFY2(!dlg->notificationAreaMessageBox->text().isEmpty(), "the refusal was not reported to the user");
    ...
    void test_profileWithOnlyAMapIsConfirmedBeforeRemoval()
    {
        ...
        QVERIFY(QDir().mkpath(utils::getMudletPath(enums::profileMapsPath, mapped)));
        QVERIFY(!QDir(utils::getMudletPath(enums::profileXmlFilesPath, mapped)).exists());

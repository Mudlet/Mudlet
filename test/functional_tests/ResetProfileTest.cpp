...
private slots:
    ...
    {
        int id = mpHost->mLuaInterpreter.startTempTrigger(qsl("test_pattern"), qsl(""), -1);
        QVERIFY(id > 0);
    ...
    {
        int id = mpHost->mLuaInterpreter.startTempAlias(qsl("^test_alias$"), qsl(""));
        QVERIFY(id > 0);
        ...
        // via sysLoadEvent)
        QVERIFY2(mpHost->getAliasUnit()->getAlias(id) == nullptr, "Temp alias should be removed after reset");
    }
    ...
    void test_tempTimersRemovedAfterReset()
    {
        ...
        QVERIFY2(mpHost->getTimerUnit()->getTimer(id) == nullptr, "Temp timer should be removed after reset");
    }
    ...
    void test_permanentTriggerSurvivesReset()
    {
        ...
        auto [id, msg] = mpHost->mLuaInterpreter.startPermSubstringTrigger(qsl("perm_trigger"), qsl(""), patterns, qsl(""));
    ...
    {
        auto [id, msg] = mpHost->mLuaInterpreter.startPermAlias(qsl("perm_alias"), qsl(""), qsl("^perm_test$"), qsl(""));
        QVERIFY2(id > 0, qPrintable(msg));
    ...
    {
        auto [id, msg] = mpHost->mLuaInterpreter.startPermTimer(qsl("perm_timer"), qsl(""), 300.0, qsl(""));
        QVERIFY2(id > 0, qPrintable(msg));
    ...
    void test_nonPersistentStopWatchRemovedAfterReset()
    {
        ...
        QVERIFY2(mpHost->getStopWatch(id) == nullptr, "Non-persistent stopwatch should be removed after reset");
    }
    ...
    void test_stopWatchCreationBlockedDuringReset()
    {
        ...
        QVERIFY2(result2.first > 0, "Stopwatch creation should work after reset completes");
    }
    ...
    void test_luaCustomGlobalsClearedAfterReset()
    {
        ...
        QVERIFY2(lua_isnil(newL, -1), "Custom Lua global should be gone after reset");
        lua_pop(newL, 1);
    }
    ...
    void test_luaStandardFunctionsAvailableAfterReset()
    {
        ...
        QVERIFY2(!lua_isnil(L, -1), "string library should be available after reset");
        lua_pop(L, 1);

        lua_getglobal(L, "table");
        QVERIFY2(!lua_isnil(L, -1), "table library should be available after reset");
        lua_pop(L, 1);
    }
    ...
    void test_luaStateIsNewAfterReset()
    {
        ...
        lua_State* newL = mpHost->mLuaInterpreter.getLuaGlobalState();
        QVERIFY(newL);
        QVERIFY2(oldL != newL, "Lua state pointer should change after reset (new state allocated)");
    }
    ...
    void test_sysLoadEventFiredExactlyOnce()
    {
        ...
        QVERIFY2(!lua_isnil(L, -1), "sysLoadCounter should be set by the event handler");
    ...
    {
        QVERIFY2(!mpHost->mResetProfile, "mResetProfile should be false before reset");

        mpHost->resetProfile_phase1();
        QVERIFY2(mpHost->mResetProfile, "mResetProfile should be true after phase1");

        QCoreApplication::processEvents();
        QVERIFY2(!mpHost->mResetProfile, "mResetProfile should be false after phase2");
    }
    ...
    void test_mBlockScriptCompileFalseAfterReset()
    {
        ...
        QVERIFY2(!mpHost->mBlockScriptCompile, "mBlockScriptCompile should be false after reset");
    }

    void test_mEmergencyStopNotSetDuringReset()
    {
        QVERIFY2(!mpHost->mEmergencyStop, "mEmergencyStop should be false before reset");
        ...
        QCoreApplication::processEvents();
        QVERIFY2(!mpHost->mEmergencyStop, "mEmergencyStop should stay false after phase2");
    }
    ...
    {
        auto [ok, msg] = mpHost->createMiniConsole(qsl("main"), qsl("test_mc"), 0, 0, 100, 100);
        QVERIFY2(ok, qPrintable(msg));
        QVERIFY(mpHost->mpConsole->mSubConsoleMap.contains(qsl("test_mc")));

        performReset();

        QVERIFY2(!mpHost->mpConsole->mSubConsoleMap.contains(qsl("test_mc")), "Mini console should be removed after reset");
    }

    void test_labelsRemovedAfterReset()
    {
        auto [ok, msg] = mpHost->createLabel(qsl("main"), qsl("test_label"), 0, 0, 100, 100, true, false);
        QVERIFY2(ok, qPrintable(msg));
        QVERIFY(mpHost->mpConsole->mLabelMap.contains(qsl("test_label")));

        performReset();

        QVERIFY2(!mpHost->mpConsole->mLabelMap.contains(qsl("test_label")), "Label should be removed after reset");
    }
    ...
    {
        int triggerId = mpHost->mLuaInterpreter.startTempTrigger(qsl("deferred_test"), qsl(""), -1);
        QVERIFY(triggerId > 0);

        mpHost->resetProfile_phase1();

        // Phase2 has NOT run yet
        QVERIFY2(mpHost->mResetProfile, "mResetProfile should be true before processEvents");
        QVERIFY2(countTempTriggers() > 0, "Temp triggers should still exist before processEvents");

        QCoreApplication::processEvents();

        QVERIFY2(!mpHost->mResetProfile, "mResetProfile should be false after processEvents");
        QCOMPARE(countTempTriggers(), 0);
    }
    ...
    void test_geyserFrameworkAvailableAfterReset()
    {
        ...
        QVERIFY2(!lua_isnil(L, -1), "Geyser framework should be loaded after reset");
        lua_pop(L, 1);
    }
    ...
    void test_gmcpTableFreshAfterReset()
    {
        ...
        QVERIFY2(lua_isnil(newL, -1), "gmcp.testField should be nil after reset (fresh table)");
        lua_pop(newL, 2);
    }
    ...
    {
        mpHost->mLuaInterpreter.startTempTrigger(qsl("double_reset_test"), qsl(""), -1);
        ...
        QVERIFY2(!mpHost->mResetProfile, "mResetProfile should be false after reset");
        QCOMPARE(countTempTriggers(), 0);
    }
    ...
    void test_permanentScriptEventHandlerReregisteredAfterReset()
    {
        ...
        QVERIFY2(mpHost->mEventHandlerMap.contains(eventName), "Event handler should be registered before reset");

        performReset();

        QVERIFY2(mpHost->mEventHandlerMap.contains(eventName), "Event handler should be re-registered after reset via compileAll");
    }

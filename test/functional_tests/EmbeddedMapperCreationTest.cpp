...
private slots:
    ...
    void test_createMapperTakesOverFromAClosedMapWidget()
    {
        ...
        QVERIFY(mpHost->mpConsole->mapWidgetCreated());
        QVERIFY2(mapWidgetOnScreen(), "the map widget did not come up, so closing it below proves nothing");

        auto [closed, closeMessage] = mpHost->closeMapWidget();
        QVERIFY2(closed, qPrintable(closeMessage));
        QVERIFY2(!mapWidgetOnScreen(), "closeMapWidget() left the map widget on screen");
    ...
    void test_createMapperTakesOverADockHiddenByTheToolbarMapAction()
    {
        ...
        QVERIFY2(mpHost->mpConsole->mapWidgetCreated(), "the toolbar action destroyed the profile's own map dock rather than hiding it");
        QVERIFY2(!mapWidgetOnScreen(), "the toolbar action left the profile's own map dock on screen");
    ...
    void test_createMapperStillRefusesAnOpenMapWidget()
    {
        auto [opened, openMessage] = mpHost->openMapWidget(QString(), -1, -1, -1, -1);
        QVERIFY2(opened, qPrintable(openMessage));
        QVERIFY2(mapWidgetOnScreen(), "the map widget did not come up, so this covers nothing");
    ...
    // TMainConsole::mapWidget() reads the dock's own hidden state rather than its
    ...
    void test_createMapperStillRefusesAMapWidgetWhileTheMainWindowIsHidden()
    {
        ...
        QVERIFY2(mapWidgetOnScreen(), "a main window that is merely not on screen made the map widget itself count as closed");
    ...
    void test_createMapperTakesOverAMapWidgetThatWasDrawingTheMap()
    {
        ...
        mpHost->mpMap->mpMapper = nullptr;
        mudlet::self()->slot_showMapperDialog();
        QVERIFY2(mpHost->mpConsole->mapWidgetCreated(), "no map widget was built, so this covers nothing");
...
private:
    ...
    // What Host::mapWidgetGeometry(), behind the Lua getMapWidgetGeometry(), asks:
    // TMainConsole::mapWidget(), which is private, so this is the reachable
    // reading of "the profile has a map widget on screen right now".
    bool mapWidgetOnScreen() const { return mpHost->mapWidgetGeometry().has_value(); }

...
private:
    ...
    {
        return QDir(utils::getMudletPath(enums::profileXmlFilesPath, profileName)).entryList(QStringList{qsl("*.xml")}, QDir::Files);
    }
    ...
    QString profileFilePath(const QString& relativePath) const { return qsl("%1/%2").arg(utils::getMudletPath(enums::profileHomePath, mProfileName), relativePath); }
...
private slots:
    ...
    void test_refusingAnArchiveOnlyRemovesItsOwnFolder()
    {
        ...
        const QString profileHome = utils::getMudletPath(enums::profileHomePath, mProfileName);

...
if ("$Env:APPVEYOR_REPO_TAG" -eq "false" -and -Not $Env:MUDLET_VERSION_BUILD.StartsWith('-public-test-build')) {
  ...
  if ($Env:MUDLET_VERSION_BUILD.StartsWith('-public-test-build')) {
    ...
    dblsqd release -a mudlet -c public-test-build -m "(test release message here)" "${Env:VERSION}${Env:MUDLET_VERSION_BUILD}"

    Write-Output "=== Registering release with Dblsqd ==="
    dblsqd push -a mudlet -c public-test-build -r "${Env:VERSION}${Env:MUDLET_VERSION_BUILD}" -s mudlet --type "standalone" --attach win:x86 "${DEPLOY_URL}"
  } else {
    Write-Output "=== Registering release with Dblsqd ==="

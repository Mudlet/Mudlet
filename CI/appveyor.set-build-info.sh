...
if [ "${APPVEYOR_REPO_TAG}" = "false" ]; then
    # Build was NOT initiated by pushing a tag to the repository - which is
    # required for a release build - so this is one of EITHER: a PR or merge
    # testing (both are "dev"elopment types) OR a "ptb" (daily, scheduled) build
    if [ "${APPVEYOR_SCHEDULED_BUILD}" = "true" ]; then
        ...
        if [ "${COMMIT_DATE}" -lt "${YESTERDAYS_DATE}" ]; then
            # There hasn't been any changes in last 24 hours, so we need to set
            # this variable so everything can abort without raising any errors:

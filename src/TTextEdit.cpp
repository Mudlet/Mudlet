    ...
    if (lineIndex < static_cast<int>(mpBuffer->buffer.size())) {
        if (tCharIndex < static_cast<int>(mpBuffer->buffer[lineIndex].size())) {
            if (mpBuffer->buffer.at(lineIndex).at(tCharIndex).linkIndex()) {
                ...
                QStringList tooltip = mpHost->mLinkStore.getHints(mpBuffer->buffer.at(lineIndex).at(tCharIndex).linkIndex());
        ...
        if (y < static_cast<int>(mpBuffer->buffer.size())) {
            if (x < static_cast<int>(mpBuffer->buffer[y].size())) {
                if (mpBuffer->buffer.at(y).at(x).linkIndex()) {
                    QStringList command = mpHost->mLinkStore.getLinks(mpBuffer->buffer.at(y).at(x).linkIndex());
                    QString func;
                    if (!command.empty()) {
                        func = command.at(0);
    ...
    if (event->button() == Qt::RightButton) {
        ...
        if (y < static_cast<int>(mpBuffer->buffer.size())) {
            if (x < static_cast<int>(mpBuffer->buffer[y].size())) {
                if (mpBuffer->buffer.at(y).at(x).linkIndex()) {
                    QStringList command = mpHost->mLinkStore.getLinks(mpBuffer->buffer.at(y).at(x).linkIndex());
                    QStringList hint = mpHost->mLinkStore.getHints(mpBuffer->buffer.at(y).at(x).linkIndex());
                    if (command.size() > 1) {

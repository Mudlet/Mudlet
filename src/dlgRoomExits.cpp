...
void dlgRoomExits::save()
{
    ...
    for (int i = 0; i < specialExits->topLevelItemCount(); i++) {
        ...
        if (value != tr("(command or Lua script)", "Placeholder, if a special exit has no code given, yet. This string is also used programmatically - ensure all five instances are the same") && key != 0 && mpHost->mpMap->mpRoomDB->getRoom(key) != nullptr) {

...
TBuffer TBuffer::copy(QPoint& P1, QPoint& P2)
{
    ...
    for (int total = P2.x(); x < total; ++x) {
        ...
        if (linkId && (linkId != oldLinkId)) {
            id = slice.mLinkStore.addLinks(mpHost, mLinkStore.getLinksConst(linkId), mLinkStore.getHintsConst(linkId));
            oldLinkId = linkId;
        }
...
void TBuffer::appendBuffer(const TBuffer& chunk)
{
    ...
    for (int cx = 0, total = static_cast<int>(chunk.buffer.at(0).size()); cx < total; ++cx) {
        ...
        if (linkId && (oldLinkId != linkId)) {
            id = mLinkStore.addLinks(mpHost, chunk.mLinkStore.getLinksConst(linkId), chunk.mLinkStore.getHintsConst(linkId));
            oldLinkId = linkId;
        }
        if (!linkId) {
            id = 0;

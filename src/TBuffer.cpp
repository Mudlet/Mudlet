...
const QMap<QPair<QString, QString>, QVector<QChar>> TBuffer::csmEncodingTable = {
    {QPair(QLatin1String("ISO 8859-2"), QLatin1String(tr("ISO 8859-2 (Central European)"))),
...
constexpr const QList<QString> TBuffer::getFriendlyEncodingNames() {
    QList encodings;
    for (auto keypair: csmEncodingTable.keys()) {
        encodings << keypair.second();
    }

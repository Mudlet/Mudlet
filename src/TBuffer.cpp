...
, mMainIncomingCodec(nullptr)
{
    ...
    // things:
    for (const auto& tableName : csmEncodingTable.getEncodingNames()) {
        Q_ASSERT_X(csmEncodingTable.getLookupTable(tableName).size() == 128, "TBuffer", "Mis-sized encoding look-up table.");
    }
#endif
}
...
void TBuffer::translateToPlainText(std::string& incoming, const bool isFromServer)
{
    ...
    const QVector<QChar>& encodingLookupTable = csmEncodingTable.getLookupTable(mEncoding);
...
COMMIT_LINE:
        ...
        if (mMxpTagProcessor.isInLinkMode()) {

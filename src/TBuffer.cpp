...
void TBuffer::updateColors()
{
    ...
    mColorSettings.updateColorSettings(pH->mColorSettings);
}
            ...
            if (cFinal.indexOf(localBuffer[spanEnd]) >= 0) {
                ...
                case static_cast<quint8>('C'): {
                    ...
                    if (isOk && spacesNeeded > 0) {
                        ...
                        TChar c = mColorSettings.createTransparent();
...
COMMIT_LINE:
        ...
        if (!encodingLookupTable.isEmpty()) {
         ...
         TChar c = mColorSettings.createChar();
...
void TBuffer::decodeSGR(const QString& sequence)
{
    ...
    mColorSettings.decodeSGR(sequence, pHost->getHaveColorSpaceId(), pHost->mColorSettings);
}

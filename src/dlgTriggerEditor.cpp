...
, mAutosaveInterval{}
{
    ...
    msgInfoAddKey = tr("<p>To add a new key binding:"
                       ...
                       "<i>NOTE: If you want to bind a key combination you may need to hold down the modifier keys (e.g. <tt>Control</tt>, <tt>Shift</tt> etc.) down before clicking on grab key.</i></li>"
                       "<li><b>3.</b> Define a command that is executed when the key is hit or write a script for more complicated needs.</li>"
                       "<li><b>4. <u>Activate</u></b> the new key binding.</li>"
                       "<li><b>5.</b> if you wish to adjust the modifiers subsequently you may click on the check boxes but be aware that some modifiers may not be available on all operating systems or may be used for some keys on an operating system outside of a user's control.</ul></p>"
                       "<p>Check the manual for <a href='http://wiki.mudlet.org/w/Manual:Contents'>more information</a>.</p>");
...
void dlgTriggerEditor::key_grab_callback(const int key, const Qt::KeyboardModifiers modifiers)
{

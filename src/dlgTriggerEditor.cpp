...
, mpHost( pH )
{
    ...
    QAction * viewVarsAction = new QAction(QIcon(":/icons/preferences-desktop-keyboard.png"), tr("Variables"), this);
...
void dlgTriggerEditor::saveVar(){
    ...
    if (varRecast == 2){
        //qDebug()<<"it works out";
        if ( var ){
            if ( newName == var->getName() && newValue == var->getValue() )
            {
                qDebug()<<"no change";
            }
        }

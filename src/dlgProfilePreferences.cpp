...
void dlgProfilePreferences::initWithHost(Host* pHost)
{
    ...
    // This will also settle utils::usingMudletDictionaries():
    const QString path = utils::getMudletPath(enums::hunspellDictionaryPath, currentDictionary);
    ...
    */
    ...
    lineEdit_logFileFolder->setPlaceholderText(utils::getMudletPath(enums::profileReplayAndLogFilesPath, pHost->getName()));
    ...
    const QStringList profileList = QDir(utils::getMudletPath(enums::profilesPath)).entryList(QDir::Dirs | QDir::NoDotAndDotDot, QDir::Time); // sort by profile "hotness"
...
void dlgProfilePreferences::fillOutMapHistory()
{
    ...
    QDir mapSaveDir(utils::getMudletPath(enums::profileMapsPath, profile_name).append(QLatin1Char('/')));
...
{
    const QString mapsPath = utils::getMudletPath(enums::profileMapsPath, pHost->getName());
    const QDir mapsDir = QDir(mapsPath);
    return mapsDir.exists() ? mapsPath : utils::getMudletPath(enums::profileHomePath, pHost->getName());
}
...
void dlgProfilePreferences::slot_copyMap()
{
    ...
    while (itAction.hasNext()) {
        ...
        if (_action->isChecked()) {
            ...
            const QString toProfileDirPathString = utils::getMudletPath(enums::profileHomePath, pHost->getName());
    ...
    const QString sourceMapFolder(utils::getMudletPath(enums::profileMapsPath, pHost->getName()));
    ...
    while (itOtherProfile.hasNext()) {
        ...
        if (!thisProfileLatestMapFile.copy(utils::getMudletPath(enums::profileMapPathFileName, otherHostName, thisProfileLatestMapPathFileName))) {
...
void dlgProfilePreferences::slot_setLogDir()
{
    ...
    if (!currentLogDir.isEmpty() && currentLogDir != nullptr) {
        ...
        // default path
        if (currentLogDir == utils::getMudletPath(enums::profileReplayAndLogFilesPath, pHost->getName())) {
...
void dlgProfilePreferences::maybeDownloadEditorThemes()
{
    ...
    auto themesAge = QFileInfo(utils::getMudletPath(enums::editorWidgetThemeJsonFile)).lastModified().toUTC();
    ...
    connect(getReply,
            ...
            std::bind(
                    [=, this](QNetworkReply* reply) {
                        ...
                        auto future = QtConcurrent::run(mudlet::unzip, tempThemesArchive->fileName(), utils::getMudletPath(enums::mainDataItemPath, qsl("edbee/")), temporaryDir.path());
...
{
    QFile themesFile(utils::getMudletPath(enums::editorWidgetThemeJsonFile));

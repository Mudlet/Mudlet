...
void markQString(QString* text)
{
    ...
    if (trimmedText.front() == '^') {
        auto firstNonSpace = std::find_if(text->begin(), text->end(), [](char c) {return c != ' '});
        std::replace(text->begin(), firstNonSpace, ' ', middleDot);
    }

    // Mark trailing spaces after $ with a middle dot
    if (trimmedText.back() == '$') {
        auto lastNonSpace = std::find_if(text->rbegin(), text->rend(), [](char c) {return c != ' '});
        std::replace(text->rend() - lastNonSpace, text->end(), ' ', middleDot);
    }
}

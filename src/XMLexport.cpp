...
void XMLexport::writeHost(Host* pHost, pugi::xml_node mudletPackage)
{
    ...
    host.append_attribute("shareScreenReader") = QMetaEnum::fromType<ClientVariables::DataSharingBehaviour>().valueToKey(
            static_cast<int>(pHost->mpClientVariables->mShareScreenReader));
    host.append_attribute("shareSystemType") = QMetaEnum::fromType<ClientVariables::DataSharingBehaviour>().valueToKey(
            static_cast<int>(pHost->mpClientVariables->mShareSystemType));
    host.append_attribute("shareUser") = QMetaEnum::fromType<ClientVariables::DataSharingBehaviour>().valueToKey(
            static_cast<int>(pHost->mpClientVariables->mShareUser));

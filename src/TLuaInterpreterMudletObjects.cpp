...
int TLuaInterpreter::permAlias(lua_State* L)
{
    ...
    if (lua_istable(L, 1)) {
        QString name, parent, regex, script;
        bool hasName = false, hasParent = false, hasRegex = false, hasScript = false;
        ...
        while (lua_next(L, 1) != 0) {
            ...
            if (!key.compare(QLatin1String("name"), Qt::CaseInsensitive) ||
                !key.compare(QLatin1String("aliasName"), Qt::CaseInsensitive)) {
                ...
                regex = getVerifiedString(L, __func__, -1, key);
                hasRegex = true;
                ...
                if (auto [validationResult, validationMessage] = pLuaInterpreter->validateLuaCodeParam(-1); !validationResult) {
                    lua_pop(L, 2);
                    lua_pushfstring(L, "permAlias: bad script value (%s)", validationMessage.toUtf8().constData());
                    return lua_error(L);
                }
        ...
        if (!hasRegex) {
            return warnArgumentValue(L, __func__, "missing required 'regex' in table");
        }
        if (!hasScript) {
    ...
    if (lua_istable(L, 1) && lua_gettop(L) == 1) {
        ...
        while (lua_next(L, 1) != 0) {
            ...
            if (!key.compare(QLatin1String("name"), Qt::CaseInsensitive) ||
                ...
                if (auto [validationResult, validationMessage] = pLuaInterpreter->validateLuaCodeParam(-1); !validationResult) {
                    lua_pop(L, 2);
                    lua_pushfstring(L, "permBeginOfLineStringTrigger: bad script value (%s)", validationMessage.toUtf8().constData());
                    return lua_error(L);
                }
                script = QString::fromUtf8(lua_tostring(L, -1));
                hasScript = true;
        ...
        Host& host = getHostFromLua(L);
        TLuaInterpreter* pLuaInterpreter = host.getLuaInterpreter();
        auto [triggerId, message] = pLuaInterpreter->startPermBeginOfLineStringTrigger(name, parent, patterns, script);
        if (triggerId == -1) {
...
int TLuaInterpreter::tempComplexRegexTrigger(lua_State* L)
{
    ...
    if (lua_istable(L, 1)) {
        ...
        while (lua_next(L, 1) != 0) {
            ...
            } else if (!key.compare(QLatin1String("highlightBgColor"), Qt::CaseInsensitive) ||
            ...
            } else if (!key.compare(QLatin1String("soundFile"), Qt::CaseInsensitive) ||

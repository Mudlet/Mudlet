...
int TLuaInterpreter::permKey(lua_State* L)
{
    ...
    if (lua_gettop(L) > 4) {
        keyModifiers = static_cast<Qt::KeyboardModifiers>(getVerifiedInt(L, __func__, 3, "key modifier", true));
        argIndex++;
    }
    ...
    const auto required = static_cast<Qt::KeyboardModifiers>(keyModifiers & (Qt::ShiftModifier|Qt::ControlModifier|Qt::AltModifier|Qt::MetaModifier|Qt::KeypadModifier|Qt::GroupSwitchModifier));
    const auto avoided = static_cast<Qt::KeyboardModifiers>((keyModifiers << 8) & (Qt::ShiftModifier|Qt::ControlModifier|Qt::AltModifier|Qt::MetaModifier|Qt::KeypadModifier|Qt::GroupSwitchModifier));
    auto [keyID, message] = pLuaInterpreter->startPermKey(keyName, parentGroup, keyCode, std::pair<Qt::KeyboardModifiers, Qt::KeyboardModifiers>{required, avoided}, luaFunction);
    if(keyID == - 1) {
...
int TLuaInterpreter::tempKey(lua_State* L)
{
    ...
    Qt::KeyboardModifiers keyModifiers = Qt::NoModifier;
    if (lua_gettop(L) > 2) {
        keyModifiers = static_cast<Qt::KeyboardModifiers>(getVerifiedInt(L, __func__, 1, "key modifiers", true));
        argIndex++;
    }
    int keyCode = getVerifiedInt(L, __func__, argIndex, "key code");
    ...
    if (lua_isfunction(L, ++argIndex)) {
        const int result = pLuaInterpreter->startTempKey(std::pair<Qt::KeyboardModifiers, Qt::KeyboardModifiers>{required, avoided}, keyCode, QString());
        if (result == -1) {
            lua_pushnumber(L, -1);
            return 2;
        }
    ...
    const int timerID = pLuaInterpreter->startTempKey(std::pair<Qt::KeyboardModifiers, Qt::KeyboardModifiers>{required, avoided}, keyCode, luaFunction);

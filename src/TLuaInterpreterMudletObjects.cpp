...
int TLuaInterpreter::permColorTrigger(lua_State* L)
{
    ...
    QStringList regList;
    if (!lua_istable(L, 3)) {
        lua_pushfstring(L, "%s: bad argument #3 type (foreground and background color pair sub-table list as table expected, got %s!)",
    ...
    auto errorOutOnWrongType = [&](lua_State* pLuaState, const char* functionName, const int mainTableIndex, const int subTableIndex, const char* argumentTypeName){
        lua_pushfstring(pLuaState,
                        ...
                        functionName, mainTableIndex, subTableIndex,
                        argumentTypeName);
        lua_error(pLuaState);
        Q_UNREACHABLE();
    };
    ...
    while (lua_next(L, 3) != 0) {
        ...
        if (lua_type(L, -1) != LUA_TTABLE) {
            lua_pushfstring(L, "%s: bad argument #3 table item index #%d type (text attributes list must be a table containing tables of zero, one (foreground) or two color (foreground, background) numbers, followed by additional strings for other format attributes {NOT currently supported}, got %s as indicated item!)",
                            __func__, i, luaL_typename(L, -1));
            lua_error(L);
            Q_UNREACHABLE();
        }
                ...
                if (lua_type(L, -1) == LUA_TNUMBER) {
                    ...
                    if (!validateColorCode(foregroundColor.value())) {
                        return warnArgumentValue(L, __func__,
                                                 qsl("number %1 is not a valid foreground color code - is it from a later version of Mudlet that handles more values?")
                                                         .arg(QString::number(foregroundColor.value())));
                    }
                ...
                if (lua_type(L, -1) == LUA_TNUMBER) {
                    if (foregroundColor.has_value()) {
                        ...
                        if (!validateColorCode(backgroundColor.value())) {
                            return warnArgumentValue(L, __func__,
                                                     qsl("number %1 is not a valid background color code - is it from a later version of Mudlet?")
                                                             .arg(QString::number(backgroundColor.value())));
                        }
...
int TLuaInterpreter::permExactMatchStringTrigger(lua_State* L)
{
    ...
    QStringList regList;
    if (!lua_istable(L, 3)) {
        lua_pushfstring(L, "%s: bad argument #3 type (exact-match strings list as table expected, got %s!)",
                        __func__, luaL_typename(L, 3));
        lua_error(L);
        Q_UNREACHABLE();
    }
...
int TLuaInterpreter::permRegexTrigger(lua_State* L)
{
    ...
    QStringList regList;
    if (!lua_istable(L, 3)) {
        lua_pushfstring(L, "%s: bad argument #3 type (regexs strings list as table expected, got %s!)",
                        __func__,
                        luaL_typename(L, 3));
        return lua_error(L);
    }
...
int TLuaInterpreter::permBeginOfLineStringTrigger(lua_State* L)
{
    ...
    QStringList regList;
    if (!lua_istable(L, 3)) {
        lua_pushfstring(L, "%s: bad argument #3 type (beginning of line strings list as table expected, got %s!)",
                        __func__,
                        luaL_typename(L, 3));
        return lua_error(L);
    }
...
int TLuaInterpreter::permSubstringTrigger(lua_State* L)
{
    ...
    QStringList regList;
    if (!lua_istable(L, 3)) {
        lua_pushfstring(L, "%s: bad argument #3 type (sub-strings list as table expected, got %s!)",
                        __func__,
                        luaL_typename(L, 3));
        return lua_error(L);
    }

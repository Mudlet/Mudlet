...
{
    // Fill properly aligned quint32 storage first; QByteArray::data() has no alignment
    // guarantee, so reinterpreting it as quint32* would be undefined behaviour.
    const int wordCount = byteCount / static_cast<int>(sizeof(quint32));
    std::vector<quint32> words(static_cast<std::size_t>(wordCount));
    QRandomGenerator::system()->fillRange(words.data(), wordCount);
    const QByteArray bytes(reinterpret_cast<const char*>(words.data()), byteCount);
...
void OAuthClientFlow::readRedirectRequest(QTcpSocket* socket)
{
    ...
    if (code.isEmpty()) {
        ...
        fail(qsl("the provider's redirect did not include an authorization code"));
        return;
    }

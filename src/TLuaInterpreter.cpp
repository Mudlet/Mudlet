...
void TLuaInterpreter::parseHintsTable(lua_State* lState, const char* functionName, int& index, QStringList& hintList)
{
    if (!lua_istable(lState, index)) {
        lua_pushfstring(lState, "%s: bad argument #%s type (%s as table expected, got %s!)", functionName, "hints", luaL_typename(lState, index));
        lua_error(lState);
        Q_UNREACHABLE();
    }
...
{
    if (!lua_istable(lState, index)) {
        lua_pushfstring(lState, "%s: bad argument #%s type (%s as table expected, got %s!)", functionName, "commands/functions", luaL_typename(lState, index));
        lua_error(lState);
        Q_UNREACHABLE();
    }

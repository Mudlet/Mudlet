...
int TLuaInterpreter::putHTTP(lua_State* L)
{
    ...
    if (mudlet::debugMode) {
        TDebug(Qt::white, Qt::blue) << "putHTTP: script is uploading data to " << reply->url().toString() << "\n" >> &host;
    }
...
int TLuaInterpreter::postHTTP(lua_State* L)
{
    ...
    if (mudlet::debugMode) {
        TDebug(Qt::white, Qt::blue) << QStringLiteral("postHTTP: script is uploading data to %1\n").arg(reply->url().toString()) >> &host;
    }

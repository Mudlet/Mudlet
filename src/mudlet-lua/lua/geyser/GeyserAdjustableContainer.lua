...
local function adjust_Info(self, label, event)
    ...
    adjustInfo = {name = adjustInfo.name or label.name, top = top, bottom = bottom, left = left, right = right, x = x, y = y, move = adjustInfo.move}
end
...
function Adjustable.Container:onMove (label, event)
    ...
    if adjustInfo.name == label.name and adjustInfo.move == nil then
        adjust_Info(self, label, event)
    end
...
function Adjustable.Container:addConnectMenu()
    ...
    local L = Adjustable.Container.Locale or {}
    local menuTxt = L.connectTo and L.connectTo.message or "Connect To: "
    ...
    for  k,v in ipairs(menuParent[#menuParent]) do
        menuTxt = L[v] and L[v].message or v
    ...
    menuTxt = L.disconnect and L.disconnect.message or "Disconnect"
...
function Adjustable.Container:resizeBorder()
    local winw, winh = getMainWindowSize()
    self.timer_active = (self.timer_active == nil) and true or self.timer_active
...
function Adjustable.Container:detach()
    ...
    self.borderSize = nil
    self.attached=false
    if oldAttached then self:resetBorder(oldAttached) end
...
function Adjustable.Container:lockContainer(lockNr, lockStyle)
    ...
    elseif type(lockNr) == "number" then
      lockStyle = self.lockStyles[lockNr] and self.lockStyles[lockNr][1]
    end
...
local function createMenus(self, parent, name, func)
    local label = self.adjLabel
    local L = Adjustable.Container.Locale or {}
    local menuTxt = L[name] and L[name].message or name
...
function Adjustable.Container:onEnterAtt()
    ...
    for i=1,#attm do
        ...
        local txt = L[attm[i]] and L[attm[i]].message or attm[i]
        self.att[i]:echo("<center>"..txt, "nocolor")
...
function Adjustable.Container:echoRightClickMenu()
    for k,v in ipairs(self.adjLabel.rightClickMenu.MenuItems) do
        if type(v) == "string" and self[v] then
            self[v]:echo(self[v].txt or "", "nocolor")
        end
    end
end
...
function Adjustable.Container:show(auto)
    ...
    if self.rCLabel then closeAllLevels(self.rCLabel) end
end
...
function Adjustable.Container:load(slot, dir)
    ...
    if mytable.windowname ~= self.windowname then
        ...
        elseif Geyser.parentWindows[mytable.windowname] then
    ...
    if mytable.auto_hidden or mytable.hidden then
        ...
        self.auto_hidden = mytable.auto_hidden
    else
        self:show()
    end
...
--- overridden reposition function
...
--- saves all adjustable containers
...
--- loads all adjustable containers
...
--- shows all adjustable containers
...
-- Internal function to create lockstyles
...
function Adjustable.Container:enableAutoSave()
    self.autoSave = true
    if not self.autoSaveHandler then
        self.autoSaveHandler = registerAnonymousEventHandler("sysExitEvent", function() self:save() end)
    end
end
...
function Adjustable.Container:new(cons,container)
    Adjustable.Container.Locale = Adjustable.Container.Locale or loadTranslations("AdjustableContainer") or {}
    local L = Adjustable.Container.Locale
    cons = cons or {}
    ...
    me.lockLabel.txt = me.lockLabel.txt or [[<font size="5" face="Noto Emoji">🔒</font>]] .. (L.lock and L.lock.message or "Lock")
    me.minLabel.txt = me.minLabel.txt or [[<font size="5" face="Noto Emoji">🗕</font>]] .. (L.min_restore and L.min_restore.message or "Min/Restore")
    me.saveLabel.txt = me.saveLabel.txt or [[<font size="5" face="Noto Emoji">💾</font>]].. (L.save and L.save.message or "Save")
    me.loadLabel.txt = me.loadLabel.txt or [[<font size="5" face="Noto Emoji">📁</font>]].. (L.load and L.load.message or "Load")
    me.attLabel.txt  = me.attLabel.txt or [[<font size="5" face="Noto Emoji">⚓</font>]].. (L.attach and L.attach.message or "Attach")
    me.lockStylesLabel.txt = me.lockStylesLabel.txt or [[<font size="5" face="Noto Emoji">🖌</font>]].. (L.lockstyle and L.lockstyle.message or "Style")
    me.customItemsLabel.txt = me.customItemsLabel.txt or [[<font size="5" face="Noto Emoji">🖇</font>]].. (L.custom and L.custom.message or "Custom")
    ...
    if me.minimized then me:minimize() end
    if me.locked then me:lockContainer() end
    if me.attached then

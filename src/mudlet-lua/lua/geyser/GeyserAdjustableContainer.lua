...
local function windowRoot(self)
  local parentWindow = self.windowname
    and self.windowname ~= "main"
...
function Adjustable.Container:dragLeavesParent(ux, uy, grabX, grabY)
  ...
  return pointerX < left - dragOutMargin
    or pointerY < top - dragOutMargin
    or pointerX > right + dragOutMargin
    or pointerY > bottom + dragOutMargin
end
...
function Adjustable.Container:dragOutOfParent(ux, uy)
  ...
  local origh = self.minimized
    and relativeToParent(self.origh)
    and constraintInPixels(self, self.origh, "height", parent)
  ...
  if relativeToParent(self.width) then
    self.width = make_percent(w / rootw)
  end
  if relativeToParent(self.height) then
    self.height = make_percent(h / rooth)
  end
  if origh then
    self.origh = make_percent(origh / rooth)
  end
...
function Adjustable.Container:deleteSaveFile(dir)
  assert(
    dir == nil or type(dir) == "string",
    "Adjustable.Container.deleteSaveFile: bad argument #1 type (directory as string expected, got " .. type(dir) .. "!)"
  )
  dir = dir or self.defaultDir

describe("PCRE regex cases with tempRegexTrigger", function()
  ...
  it(
    "selectCaptureGroup selects correct position for named groups when captured text appears earlier in line",
    function()
      local selection
      local pattern =
        "^Hp: (?<chp>[0-9\\-]+)/(?<mhp>[0-9]+) Sp: (?<csp>[0-9\\-]+)/(?<msp>[0-9]+) Ep: (?<cep>[0-9\\-]+)/(?<mep>[0-9]+) Wght: (?<pwt>[0-9\\.]+)% Gold: (?<gld>[0-9]+) Algn: (?<align>[A-Za-z]+) Wpn: (?<wpn>W|H|N)$"

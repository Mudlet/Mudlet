...
local function logOp(fmt, ...)
  ...
  if #opLog > 40 then
    table.remove(opLog, 1)
  end -- keep a short tail
...
local function wildText()
  ...
  if k == 1 then
    return string.rep(pick({ "a", "x", "9", " " }), rrange(1, 30))
  end
  if k == 2 then
    return pick({ "τ", "日本語", "αβγ", "e\xcc\x81", "🜲🜳", "\xc3\xa9" })
  end
  if k == 3 then
    return "\27[1;31mred\27[0m"
  end
  if k == 4 then
    return "\27]8;;http://example.com/" .. string.rep("a", rrange(0, 40)) .. "\27\\link\27]8;;\27\\"
  end
  if k == 5 then
    return pick({ "[", "]", "(", ")", "|", "\\", "*", ".", "^", "$", "%1", "\t\t", "\r" })
  end
...
local OPS = {
  function(win)
    local t, n = wildText(), wildInt()
    logOp("selectString %s %q %d", win, t, n)
    callWin(selectString, win, t, n)
  end,
  function(win)
    local a, b = wildInt(), wildInt()
    logOp("selectSection %s %d %d", win, a, b)
    callWin(selectSection, win, a, b)
  end,
  function(win)
    logOp("selectCurrentLine %s", win)
    callWin(selectCurrentLine, win)
  end,
  function(win)
    local t = wildText()
    logOp("replace %s %q", win, t)
    callWin(replace, win, t)
  end,
  function(win)
    logOp("deleteLine %s", win)
    callWin(deleteLine, win)
  end,
  function(win)
    local a, b = wildInt(), wildInt()
    logOp("moveCursor %s %d %d", win, a, b)
    callWin(moveCursor, win, a, b)
  end,
  function(win)
    logOp("moveCursorEnd %s", win)
    callWin(moveCursorEnd, win)
  end,
  function(win)
    local t = wildText()
    logOp("insertText %s %q", win, t)
    callWin(insertText, win, t)
  end,
  function(win)
    logOp("setFgColor %s", win)
    callWin(setFgColor, win, rrange(0, 255), rrange(0, 255), rrange(0, 255))
  end,
  function(win)
    logOp("setBgColor %s", win)
    callWin(setBgColor, win, rrange(0, 255), rrange(0, 255), rrange(0, 255))
  end,
  function(win)
    logOp("resetFormat %s", win)
    callWin(resetFormat, win)
  end,
  function(win)
    logOp("copy %s", win)
    callWin(copy, win)
  ...
  function(win)
    logOp("appendBuffer %s", win)
    callWin(appendBuffer, win)
  end,
  function(win)
    local a, b = wildInt(), wildInt()
    logOp("getLines %s %d %d", win, a, b)
    callWin(getLines, win, a, b)
  end,
  function(win)
    logOp("getColumnNumber %s", win)
    callWin(getColumnNumber, win)
  end,
  function(win)
    logOp("getLineNumber %s", win)
    callWin(getLineNumber, win)
  end,
  function(win)
    logOp("getLastLineNumber %s", win)
    callWin(getLastLineNumber, win)
  end,
  function(win)
    logOp("getCurrentLine %s", win)
    callWin(getCurrentLine, win)
  end,
  function(win)
    local n = wildInt()
    logOp("setWindowWrap %s %d", win, n)
    callWin(setWindowWrap, win, n)
  end,
  function(win)
    local n = wildInt()
    logOp("setWindowWrapIndent %s %d", win, n)
    callWin(setWindowWrapIndent, win, n)
  end,
  ...
  function(win)
    local t = "<b>" .. wildText() .. "</b>"
    logOp("insertHTML main %q", t)
    pcall(insertHTML, t)
  end,
  function(win)
    logOp("clearWindow %s", win)
    callWin(clearWindow, win)
  end,
  function(win)
    logOp("feed-more %s", win)
    seedLines(win, rrange(1, 3))
  end,
  function(win)
    local n = wildInt()
    logOp("selectCaptureGroup %s %d", win, n)
    callWin(selectCaptureGroup, win, n)
  end,
  function(win)
    local t = wildText()
    logOp("replaceLine %s %q", win, t)
    callWin(selectCurrentLine, win)
    callWin(replace, win, t)
  end,
}

...
describe("Tests mapper functions against a shared fixture", function()
  ...
  describe("Tests the 2D map room symbol font", function()
    local originalFont, originalOnlyUseSelected, originalScaling
    local otherFont

    setup(function()
      originalFont = getMapSymbolFont()
      originalOnlyUseSelected = getMapSymbolFontOnlyUseSelected()
      originalScaling = getMapSymbolFontScaling()

      -- Pick an installed font that is not the one already in use, so a
      -- set/read-back assertion cannot pass by doing nothing. pairs() over the
      -- getAvailableFonts() set has no defined order, so sort for a stable pick.
      local names = {}
    ...
    teardown(function()
      setMapSymbolFont(originalFont)
      setMapSymbolFontOnlyUseSelected(originalOnlyUseSelected)
      setMapSymbolFontScaling(originalScaling)
    end)

    it("setMapSymbolFont is read back by getMapSymbolFont", function()
      assert.is_string(otherFont)
      assert.is_true(setMapSymbolFont(otherFont))
      assert.are.equal(otherFont, getMapSymbolFont())
    end)

    it("a font name is matched case-insensitively and read back canonically", function()
      assert.is_true(setMapSymbolFont(originalFont))
      assert.is_true(setMapSymbolFont(otherFont:upper()))
      assert.are.equal(otherFont, getMapSymbolFont())
    end)

    it("an unavailable font is refused and leaves the font unchanged", function()
      assert.is_true(setMapSymbolFont(otherFont))
      local ok, err = setMapSymbolFont("No Such Font At All")
      assert.is_nil(ok)
      assert.is_string(err)
      assert.is_truthy(err:find("is not available", 1, true), err)
      assert.are.equal(otherFont, getMapSymbolFont())
    end)

    it("an empty font name is refused", function()
      local ok, err = setMapSymbolFont("   ")
      assert.is_nil(ok)
      assert.is_string(err)
      assert.is_truthy(err:find("must not be empty", 1, true), err)
    end)

    it("a font name that is not a string hard-errors", function()
      -- a number would be coerced to a string by lua_isstring(), so use a table
      assert.has_error(function() setMapSymbolFont({}) end)
    end)

    it("a non-boolean only-use-selected argument hard-errors", function()
      assert.has_error(function() setMapSymbolFontOnlyUseSelected("yes") end)
    end)

    it("setMapSymbolFontOnlyUseSelected is read back by its getter", function()
      assert.is_true(setMapSymbolFontOnlyUseSelected(true))
      assert.is_true(getMapSymbolFontOnlyUseSelected())
      assert.is_true(setMapSymbolFontOnlyUseSelected(false))
      assert.is_false(getMapSymbolFontOnlyUseSelected())
    end)

    it("toggling only-use-selected does not disturb the chosen font", function()
      assert.is_true(setMapSymbolFont(otherFont))
      assert.is_true(setMapSymbolFontOnlyUseSelected(true))
      assert.are.equal(otherFont, getMapSymbolFont())
      assert.is_true(setMapSymbolFontOnlyUseSelected(false))
    ...
    it("setMapSymbolFontScaling is read back by getMapSymbolFontScaling", function()
      assert.is_true(setMapSymbolFontScaling(1.25))
      assert.are.equal(1.25, getMapSymbolFontScaling())
    end)

    it("accepts the ends of the 0.50 to 2.00 range", function()
      assert.is_true(setMapSymbolFontScaling(0.50))
      assert.are.equal(0.50, getMapSymbolFontScaling())
      assert.is_true(setMapSymbolFontScaling(2.00))
      assert.are.equal(2.00, getMapSymbolFontScaling())
    end)

    it("a scaling factor outside the range is refused and changes nothing", function()
      assert.is_true(setMapSymbolFontScaling(1.10))
      for _, value in ipairs({0.49, 2.01, -1, 0}) do
        local ok, err = setMapSymbolFontScaling(value)
        assert.is_nil(ok)
        assert.is_string(err)
        assert.is_truthy(err:find("out of range", 1, true), err)
      end
      assert.are.equal(1.10, getMapSymbolFontScaling())
    end)

    it("a scaling factor that is not a number hard-errors", function()
      assert.has_error(function() setMapSymbolFontScaling("big") end)
    end)
  end)

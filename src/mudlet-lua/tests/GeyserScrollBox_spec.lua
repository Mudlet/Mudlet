...
describe("Tests functionality of Geyser.ScrollBox", function()
  ...
  describe("Geyser.ScrollBox:new/new2", function()
    ...
    it("starts out hidden when the constraints ask for it", function()
      local scrollBox =
        track(Geyser.ScrollBox:new({ name = "gsbHiddenNew", x = 0, y = 0, width = 50, height = 50, hidden = true }))
      assert.is_true(scrollBox.hidden)
      assert.is_false(windowVisible("gsbHiddenNew"))
      scrollBox:show()
      assert.is_false(scrollBox.hidden)
      assert.is_true(windowVisible("gsbHiddenNew"))
    end)
  end)
  ...
  describe("Geyser.ScrollBox children", function()
    ...
    it("places children in its own coordinate space", function()
      local console = track(
        Geyser.MiniConsole:new({ name = "gsbConsole", x = "10%", y = "10%", width = "80%", height = "50%" }, scrollBox)
      )
    ...
    it("re-lays its children out when it is resized", function()
      track(
        Geyser.MiniConsole:new({ name = "gsbResized", x = "10%", y = "10%", width = "80%", height = "50%" }, scrollBox)
      )
    ...
    it("nests a container of its own inside the scroll box", function()
      local inner =
        track(Geyser.Container:new({ name = "gsbInner", x = 0, y = 0, width = "50%", height = "50%" }, scrollBox))
      local label =
        track(Geyser.Label:new({ name = "gsbInnerLabel", x = "50%", y = 0, width = "50%", height = "100%" }, inner))
    ...
    it("nests a scroll box inside a scroll box, each its own parent window", function()
      local inner =
        track(Geyser.ScrollBox:new({ name = "gsbNestedBox", x = 10, y = 10, width = "50%", height = "50%" }, scrollBox))
      ...
      local label =
        track(Geyser.Label:new({ name = "gsbNestedLabel", x = "50%", y = 0, width = "50%", height = "100%" }, inner))
  ...
  describe("Geyser.ScrollBox:reposition", function()
    it("restores geometry that was changed behind Geyser's back", function()
      local scrollBox =
        track(Geyser.ScrollBox:new({ name = "gsbReposition", x = 10, y = 10, width = 100, height = 80 }))
    ...
    it("puts its own origin back to zero after repositioning", function()
      local scrollBox =
        track(Geyser.ScrollBox:new({ name = "gsbOriginKept", x = 30, y = 40, width = 100, height = 80 }))
  ...
  describe("Geyser.ScrollBox created in a hidden container", function()
    it("stays off screen, and comes back when the container is shown", function()
      ...
      parent:hide()
      local scrollBox =
        track(Geyser.ScrollBox:new2({ name = "gsbBornHidden", x = 0, y = 0, width = "100%", height = "50%" }, parent))
      local console = track(
        Geyser.MiniConsole:new2(
          { name = "gsbBornHiddenConsole", x = 0, y = "50%", width = "100%", height = "50%" },
          parent
        )
      )
    ...
    it("can be taken off screen by hand without being shown first", function()
      ...
      parent:hide()
      local scrollBox =
        track(Geyser.ScrollBox:new2({ name = "gsbHideByHand", x = 0, y = 0, width = "100%", height = "100%" }, parent))
      scrollBox:hide()
    ...
    it("stays off screen when it was asked to start hidden", function()
      local scrollBox = track(
        Geyser.ScrollBox:new2({ name = "gsbBornHiddenFlag", x = 0, y = 0, width = 100, height = 100, hidden = true })
      )
      assert.is_true(scrollBox.hidden)
  ...
  pending(
    "Geyser.ScrollBox shows Qt's own scroll bar once a child overflows it - a scroll box is not a console, so no console scroll bar getter can report it; this needs a scroll box specific getter"
  )

  pending(
    "Geyser.ScrollBox:setStyleSheet - the method is commented out in GeyserScrollBox.lua because Mudlet has no setScrollBoxStyleSheet primitive to call"
  )

describe("Tests functionality of Geyser.Button", function()
  ...
  describe('Tests the functionality of Geyser.Button:setState', function()
    ...
    it('should call echo with the message for the state set', function()
      ...
      assert.spy(echoSpy).was.called_with(match.is_same(gb), gb.msg)
      gb:setState("down")
      assert.spy(echoSpy).was.called()
      assert.spy(echoSpy).was.called_with(match.is_same(gb), gb.downMsg)
    end)

    it('should call setColor with the color for the state being set', function()
      gb:setState("up")
      assert.spy(colorSpy).was.called()
      assert.spy(colorSpy).was.called_with(gb, gb.color)
      gb:setState("down")
      assert.spy(colorSpy).was.called()
      assert.spy(colorSpy).was.called_with(gb, gb.downColor)
    end)
    ...
    it('should call setStyleSheet instead of setColor if the stylesheet is set', function()
      ...
      assert.spy(styleSheetSpy).was.called_with(gb, gb.upStyle)
      gb:setState("down")
      assert.spy(styleSheetSpy).was.called()
      assert.spy(styleSheetSpy).was.called_with(gb, gb.downStyle)
    end)

    it('should call setToolTip with the appropriate tool tip for the state being set', function()
      gb:setState("down")
      assert.spy(toolTipSpy).was.called()
      assert.spy(toolTipSpy).was.called_with(gb, gb.tooltip, gb.toolTipDuration)
      gb:setState("up")
      assert.spy(toolTipSpy).was.called()
      assert.spy(toolTipSpy).was.called_with(gb, gb.downTooltip, gb.toolTipDuration)
    end)
  end)
end)
...
describe("stt bridge", function()
  ...
  describe("getInfo", function()
    ...
    it("reports a sensitivity from the documented set", function()
      ...
      assert.is_true(
        modes[stt.getInfo().sensitivity] ~= nil,
    ...
    it("reports a state from the documented set", function()
      ...
      assert.is_true(states[stt.getInfo().state] ~= nil, "unexpected state: " .. tostring(stt.getInfo().state))
    end)
  ...
  describe("refusals", function()
    ...
    it("names where a model belongs when none is installed", function()
      if not stt.available() or #stt.listModels() > 0 then
        return
      end
      local ok, err = stt.init()
      ...
      assert.is_string(err)
      assert.is_truthy(
        err:find(stt.getModelPath(), 1, true),
        "the refusal should name the models directory, got: " .. tostring(err)
      )
    end)

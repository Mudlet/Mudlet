describe("Tests DateTime.lua functions", function()
  ...
  describe("Tests datetime:parse", function()
    it("parses the default ISO format into a date table", function()
      ...
      -- dt.day is returned as a string (the only field parse does not tonumber);
      -- tonumber tolerates both forms so the spec does not lock the quirk in
      assert.are.equal(15, tonumber(dt.day))

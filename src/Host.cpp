...
, mPort(port)
{
    ...
    mLogStatus = QFile::exists(utils::getMudletPath(enums::profileDataItemPath, mHostName, qsl("autolog")));
    // "autotimestamp" determines if profile loads with timestamps enabled
    mTimeStampStatus = QFile::exists(utils::getMudletPath(enums::profileDataItemPath, mHostName, qsl("autotimestamp")));
    ...
    const QString directoryLogFile = utils::getMudletPath(enums::profileDataItemPath, mHostName, qsl("log"));
...
#endif
            if (!mpConsole->saveMap(utils::getMudletPath(enums::profileMapPathFileName, mHostName, qsl("autosave.dat")))) {
...
void Host::loadPackageInfo()
{
    for (const auto& package : std::as_const(mInstalledPackages)) {
        const QDir dir(utils::getMudletPath(enums::profilePackagePath, getName(), package));
...
QList<Host::ModuleWriteJob> Host::prepareModuleSaves(bool backup)
{
    ...
    const QString backupPath = backup ? utils::getMudletPath(enums::moduleBackupsPath) : QString();
    ...
    while (it.hasNext()) {
        ...
        if (filename.endsWith(qsl("mpackage"), Qt::CaseInsensitive) || filename.endsWith(qsl("zip"), Qt::CaseInsensitive)) {
            xmlFilename = utils::getMudletPath(enums::profilePackagePathFileName, mHostName, moduleName);
            ...
            const QString packagePath = utils::getMudletPath(enums::profilePackagePath, mHostName, moduleName);
    ...
    while (moduleIterator.hasNext()) {
        ...
        if (moduleName == syncModuleName) {
            if (!syncingFromHost.isEmpty() && (fileName.endsWith(qsl(".zip"), Qt::CaseInsensitive) || fileName.endsWith(qsl(".mpackage"), Qt::CaseInsensitive))) {
                ...
                fileName = utils::getMudletPath(enums::profilePackagePathFileName, syncingFromHost, moduleName);
...
std::tuple<bool, QString, QString> Host::saveProfile(const QString& saveFolder, const QString& saveName, bool syncModules)
{
    ...
    if (saveFolder.isEmpty()) {
        directory_xml = utils::getMudletPath(enums::profileXmlFilesPath, getName());
    } else {
...
std::pair<bool, QString> Host::installPackage(const QString& fileName, enums::PackageModuleType thing, bool quiet)
{
    ...
    if (packageUnpacksAFolder(fileName)) {
        const QString _home = utils::getMudletPath(enums::profileHomePath, getName());
        ...
        const QString _dest = crossKindRefusalOnTheFileName.isEmpty() ? utils::getMudletPath(enums::profilePackagePath, getName(), packageName)
                                                                      : utils::getMudletPath(enums::profilePackagePath, getName(), packageName + qsl(".mudlet-installing"));
...
bool Host::uninstallPackage(const QString& packageName, enums::PackageModuleType thing)
{
    ...
    const QString dest = utils::getMudletPath(enums::profilePackagePath, getName(), packageName);
    removeDir(dest, dest);
...
{
    QSettings settings(utils::getMudletPath(enums::profileDataItemPath, getName(), qsl("profile.ini")), QSettings::IniFormat);
...
{
    QSaveFile file(utils::getMudletPath(enums::profileDataItemPath, getName(), item));
...
{
    QFile file(utils::getMudletPath(enums::profileDataItemPath, getName(), item));
...
{
    auto packagePath = utils::getMudletPath(enums::profilePackagePath, getName(), packageName);

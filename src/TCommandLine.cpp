...
{
    return is_rl_input_available;
}

char *rl_finished_line;
...
, mpUserSuggestionsList()
{
    ...
    rl_initialize();
    using_history();
    // Need to initialize separately for multiple instances to work
    rl_line_buffer = (char *) malloc(256);
    ...
    if (event->type() == QEvent::KeyPress) {
        ...
        case Qt::Key_Tab:
            ...
            if ((ke->modifiers() & (allModifiers & ~(Qt::ShiftModifier))) == Qt::ControlModifier) {
                ...
                int offset = (ke->key() == Qt::Key_Backtab) ? -1 : 1;
                int newIndex = (currentIndex + offset + count) % count;
                mudlet::self()->mpTabBar->setCurrentIndex(newIndex);
                ke->accept();
                return true; 
            } else if ((ke->modifiers() & (allModifiers & ~(Qt::ShiftModifier))) ==  Qt::NoModifier) {
                handleTabCompletion((ke->key() == Qt::Key_Tab));
                adjustHeight();
                ke->accept();
                return true;
            }
        ...
        case Qt::Key_C:
            ...
            break;
        case Qt::Key_V:
            if (mpHost->mEnableReadline && ((ke->modifiers() & allModifiers) == Qt::ControlModifier)) {
                auto clipboard = QApplication::clipboard();
                QTextCursor c = textCursor();
                QString text = clipboard->text();
                // GNU readline is about reading _lines_, so pasting endlines
                // leads to trouble.
                text = text.replace('\n', '\t');
                c.insertText(text);
                ke->accept();
        ...
        if (mpHost->mEnableReadline) {
            ...
            rl_end = fullBytes.length();
            rl_point = leftBytes.length();

            // Now feed the characters into readline
            for(int i = 0; i < keyText.length(); i++) {
                // For input_available_hook, e.g. so left-arrow = "^[[D" works correctly after C-r.
                // (as opposed to breaking with ^[, then inserting [D into the line.)
                is_rl_input_available = (i < keyText.length() - 1);

...
public:
    ...
    // Set by the performance benchmarks so a profile they create measures the
    // pipeline alone. Without it the shipped default packages - the starter UI
    // above all - end up inside the baseline they are supposed to be measured
    // against, which is how a package-driven throughput regression can hide.

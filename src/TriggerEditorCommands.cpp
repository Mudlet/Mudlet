    ...
    : QUndoCommand(parent), mCheckbox(checkbox), mOldValue(oldValue), mNewValue(newValue)
{
    QString desc = description.isEmpty() ? 
                   qsl("Change %1").arg(checkbox ? checkbox->objectName() : "checkbox") : 
                   description;
    setText(desc);
}
...
bool CheckboxChangeCommand::mergeWith(const QUndoCommand* other)
{
    ...
    const CheckboxChangeCommand* checkCmd = static_cast<const CheckboxChangeCommand*>(other);
    if (mCheckbox != checkCmd->mCheckbox) {
        return false;
    }
    
    // Merge by updating the new value (keep old value from first change)
    mNewValue = checkCmd->mNewValue;
    return true;
}
...
{
    QString desc = description.isEmpty() ? 
                   qsl("Change %1").arg(spinbox ? spinbox->objectName() : "spinbox") : 
                   description;
    setText(desc);
}
...
bool SpinboxChangeCommand::mergeWith(const QUndoCommand* other)
{
    ...
    const SpinboxChangeCommand* spinCmd = static_cast<const SpinboxChangeCommand*>(other);
    if (mSpinbox != spinCmd->mSpinbox) {
        return false;
    }
    
    // Merge by updating the new value (keep old value from first change)
    mNewValue = spinCmd->mNewValue;
    return true;
}
...
{
    QString desc = description.isEmpty() ? 
                   qsl("Change %1").arg(combobox ? combobox->objectName() : "combobox") : 
                   description;
    setText(desc);
}

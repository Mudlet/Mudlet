...
UiaWrapper::UiaWrapper()
{
    ...
    if (uiaLibrary.load()) {
        m_pUiaRaiseNotificationEvent = reinterpret_cast<PtrUiaRaiseNotificationEvent>(uiaLibrary.resolve("UiaRaiseNotificationEvent"));
        m_pUiaDisconnectAllProviders = reinterpret_cast<PtrUiaDisconnectAllProviders>(uiaLibrary.resolve("UiaDisconnectAllProviders"));
        m_pUiaDisconnectProvider = reinterpret_cast<PtrUiaDisconnectProvider>(uiaLibrary.resolve("UiaDisconnectProvider"));
        m_pUiaHostProviderFromHwnd = reinterpret_cast<PtrUiaHostProviderFromHwnd>(uiaLibrary.resolve("UiaHostProviderFromHwnd"));
        m_pUiaClientsAreListening = reinterpret_cast<PtrUiaClientsAreListening>(uiaLibrary.resolve("UiaClientsAreListening"));
    }
}
...
{
    return m_pUiaRaiseNotificationEvent && m_pUiaDisconnectAllProviders && m_pUiaDisconnectProvider && m_pUiaHostProviderFromHwnd && m_pUiaClientsAreListening;
}

HRESULT UiaWrapper::raiseNotificationEvent(IRawElementProviderSimple* provider, NotificationKind notificationKind, NotificationProcessing notificationProcessing, BSTR displayString, BSTR activityId)
{
...
HRESULT
UiaWrapper::hostProviderFromHwnd(HWND hwnd, IRawElementProviderSimple** ppProvider)
{

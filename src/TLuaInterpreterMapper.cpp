...
// Documentation: https://wiki.mudlet.org/w/Manual:Lua_Functions#getMapSymbolFont
int TLuaInterpreter::getMapSymbolFont(lua_State* L)
{
    const Host& host = getHostFromLua(L);
    if (host.mpMap.isNull()) {
        return warnArgumentValue(L, __func__, "no map loaded");
...
// Documentation: https://wiki.mudlet.org/w/Manual:Lua_Functions#setMapSymbolFont
int TLuaInterpreter::setMapSymbolFont(lua_State* L)
{
    const QString fontName = getVerifiedString(L, __func__, 1, "font name");
    if (fontName.trimmed().isEmpty()) {
        return warnArgumentValue(L, __func__, "font name must not be empty");
    }
    ...
    // Match case-insensitively but store the family as the font database spells
    // it, so that getMapSymbolFont() reads back a canonical name:
...
// Documentation: https://wiki.mudlet.org/w/Manual:Lua_Functions#getMapSymbolFontOnlyUseSelected
int TLuaInterpreter::getMapSymbolFontOnlyUseSelected(lua_State* L)
{
    const Host& host = getHostFromLua(L);
    if (host.mpMap.isNull()) {
        return warnArgumentValue(L, __func__, "no map loaded");
...
// Documentation: https://wiki.mudlet.org/w/Manual:Lua_Functions#setMapSymbolFontOnlyUseSelected
int TLuaInterpreter::setMapSymbolFontOnlyUseSelected(lua_State* L)
{
    const bool onlyUseSelectedFont = getVerifiedBool(L, __func__, 1, "only use selected font");
    ...
    host.mpMap->setOnlySymbolFontUsed(onlyUseSelectedFont);
    lua_pushboolean(L, true);
    return 1;
}

// Documentation: https://wiki.mudlet.org/w/Manual:Lua_Functions#getMapSymbolFontScaling
int TLuaInterpreter::getMapSymbolFontScaling(lua_State* L)
{
    const Host& host = getHostFromLua(L);
    if (host.mpMap.isNull()) {
        return warnArgumentValue(L, __func__, "no map loaded");
...
// Documentation: https://wiki.mudlet.org/w/Manual:Lua_Functions#setMapSymbolFontScaling
int TLuaInterpreter::setMapSymbolFontScaling(lua_State* L)
{
    const double scaling = getVerifiedDouble(L, __func__, 1, "scaling factor");
    // Matches the range of the preferences dialog spin-box:
    if (scaling < 0.50 || scaling > 2.00) {
        return warnArgumentValue(L, __func__, qsl("scaling factor %1 is out of range, it must be between 0.50 and 2.00").arg(scaling));
    }
